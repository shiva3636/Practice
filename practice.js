...
//   .map((el) => Array(9).fill(""));
// board.map((item,i) => {
//   return item.map((el,j) => grid[i][j] =el)
// })
// let arr = board[0];
for (let i = 0; i < 9; i++) {
  // let hCopy = [];
  // let hRes = [];
  // for (let j = 0; j < 9; j++) {
  //   hCopy.push(board[i][j]);
  //   if (hCopy.includes(board[i][j + 1])) {
  //     if (board[i][j + 1] != ".") {
  //       hRes.push(board[i][j + 1]);
  //     }
  //   }
  // }
  // console.log("H Copy: ", hCopy, "\nH Res: ", hRes);
  //   let vCopy = [];
  //   let vRes = [];
  //   for (let k = 0; k < 9 - 1; k++) {
  //     vCopy.push(board[k][i]);
  //     let a = board[k + 1][i];
  //     if (vCopy.includes(a)) {
  //       if (board[k + 1][i] != ".") {
  //         vRes.push(board[k + 1][i]);
  //       }
  //     }
  //   }
  //   console.log("V Copy: ", vCopy, "\nV Res:  ", vRes);
}
